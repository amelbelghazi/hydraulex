...
<div class="form-group has-feedback extraspace">
  ...
  <div class="col-sm-10">
    <select type="select" class="form-control" name="devi_id">
      <option selected="true" value="<?= h($devi->id)?>">
        <?= h($devi->intitule)?>
      </option>
    </select> 
  </div> 
            ...
            <?php if($partie->has('articles')){
             foreach($partie->articles as $article) {?>
              <tr>
                ...
                ?>
                <td class="col-sm-1 no-padding">
                  <input placeholder="Cumul" class="form-control Cumul" name="DetailsArticle[{<?= $nbarticle?>}][Cumul]" type="text" value="<?= $sum ?>">
                </td>
                <td class="col-sm-1 no-padding">
                  <input placeholder="Qte du mois" class="form-control QteMois" name="DetailsArticle[{<?= $nbarticle?>}][qtemois]" type="text">
                </td>
                <td class="col-sm-1 no-padding">
                  <input placeholder="Total mois" class="form-control TotalMois" name="DetailsArticle[{<?= $nbarticle?>}][total]" type="text">
                </td>
                <td class="col-sm-1 no-padding">
